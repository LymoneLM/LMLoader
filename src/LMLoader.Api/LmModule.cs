...
/// <summary>
/// 模组模块基类。模块是模组的最小加载单位,由 mod.json 的 <c>entry.modules[].type</c> 显式声明,
/// 加载器反射实例化(要求有无参构造函数)。依赖只声明在 mod.json,不做代码特性双声明。
...
public abstract class LmModule
{
	...
	/// <summary>取回最后注册的该类型跨模组服务;不存在返回 false。</summary>
	...
	/// 该阶段异常仅导致本模块跳过后续阶段(宽松失败传播),不影响无依赖模块。
	/// </summary>
	public virtual void OnPreLoad() { }

	/// <summary>
	/// 模块主入口;HarmonyX/MonoMod 环境已就绪,可执行 patch。
	/// 该阶段异常仅导致本模块跳过后续阶段(宽松失败传播)。
	/// </summary>
	public virtual void OnLoad() { }

	/// <summary>
	/// 全部模块 Load 完成后统一触发,用于跨模组后置协调(如读取其他模组注册的 API)。
	/// 仅对 PreLoad/Load 均成功的模块执行。
	/// </summary>
